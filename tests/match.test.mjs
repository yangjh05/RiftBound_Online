...
test("constructed decks require exactly 40 Main Deck cards and at most 8 Sideboard cards", () => {
  const deck = structuredClone(rawDecklists.originsKaisa);
  assert.equal(deck.main.reduce((sum, [, count]) => sum + count, 0), DECK_RULES.mainExact);
...
test("best-of-three sideboard match lets the loser choose first and ends at two wins", () => {
  ...
  assert.equal(beginNextMatchGame(match).firstPlayerId, "p2");
